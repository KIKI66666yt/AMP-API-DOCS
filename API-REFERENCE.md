# AMP API Reference (205 methods)

Generated from `Core/GetAPISpec` called with an authenticated admin session
(anonymous calls only return a ~10-method stub — see README.md). This is the
complete authenticated surface across 7 modules. Everything below is
spec-derived (param names/types/required permissions) — most of it has been
cross-checked against real behavior, but not every single method has been
individually curl-tested. See GOTCHAS.md for confirmed quirks and
...
Body: {"SESSIONID":"<uuid>", <param1>:..., <param2>:...}
```

Complex-typed params (`IsComplexType: true`, e.g. `InstanceDatastore`) need a nested
JSON object matching that type's shape — not discoverable from this spec dump alone;
inspect via `performance.getEntriesByType('resource')` browser capture if the panel UI
exercises that call, or trial-and-error with an empty `{}` first to read the resulting
validation error (AMP's error messages are field-specific).
